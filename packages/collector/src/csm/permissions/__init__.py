...
class PendingDecisions:
    ...
    async def register(self, request_id: int) -> asyncio.Event:
        ...
        async with self._lock:
            self._holders[request_id] = {"event": event, "decision": None, "reason": None}
        return event

    async def deliver(self, request_id: int, decision: str, reason: str | None) -> bool:
        """Return True if a waiter was present and signalled, False if no
        such request_id is registered (already timed out, or out-of-band
        decision before the receiver ever awaited)."""
        async with self._lock:
    ...
    async def cancel(self, request_id: int) -> None:
        """Drop the registration on timeout — no waiter pending anymore."""
        async with self._lock:
...
async def request_decision(
    ...
    *,
    ...
    try:
        ...
        # Mark the row timed_out; cancel the registration.
...
async def record_decision(
    ...
    *,
    ...
    if rowcount == 0:
        return False
    delivered = await decisions.deliver(request_id, decision, reason)
    # delivered=False is fine — happens when the receiver already timed
    # out and cleaned up. The row update still happened (decision is
    # recorded for audit), the caller just learns there's no live
    # awaiter.
    _ = delivered
    return True
