 ...
 *
 * Rendered as a Window so the project label sits in a small title bar
 * with a tappable "→ open project" link as the right-aligned action.
 */
export default function ProjectTreeSection({
  ...
  return (
    <Window
      icon="agents"
      title={projectLabel}
      aria-label={`${projectLabel} tree`}
      actions={
        <Link
          to={`/projects/${encodeURIComponent(worktreeRoot)}`}
          className="inline-flex items-center min-h-7 px-2 text-[11px] text-ink-muted hover:text-ink rounded-sm hover:bg-surface-2"
        >
    ...
    >
      {loading ? (
        ...
        <p className="text-xs text-ink-muted px-3 py-3">No live sessions.</p>
      ) : (
        <Tree<TreeRowData>
          data={rows}
          ref={treeRef}
