...
export function ProjectSummarySkeleton() {
  return (
    <section className="grid grid-cols-2 gap-3 rounded-md border border-line bg-surface p-3">
      {[0, 1, 2, 3].map((i) => (
...
export function TreeSkeleton({ rows = 4 }: { rows?: number }) {
  return (
    <div className="rounded-md border border-line bg-surface p-2 space-y-2">
      {Array.from({ length: rows }).map((_, i) => (
        <div
          // biome-ignore lint/suspicious/noArrayIndexKey: pure presentational placeholders
          key={i}
